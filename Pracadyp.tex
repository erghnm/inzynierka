...
\begin{equation}
L(m,n) = \left[ \begin{array}{cccc} L(1,1) & L(1,2) & \dots & L(1,N) \\ L(2,1) & L(2,2) & \dots & L(2,N) \\ \vdots & \vdots &  \ddots & \vdots \\L(M,1) & L(M,2) & \dots & L(M,N) \end{array} \right]
\end{equation}
...
\begin{figure}[b!]
  ...
  \centering\includegraphics[scale= 0.55]{figures/bin}
	\vspace{0.5cm}
  \caption[Przyk³ad obrazu binarnego]{Przyk³ad obrazu binarnego}
	\source{\url{http://earthexplorer.usgs.gov/}, adaptacja w³asna}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%\newpage
\section{Podzia³ algorytmów przetwarzania obrazów}
Celem cyfrowego przetwarzania obrazów jest dokonanie takich przekszta³ceñ na obrazie wejœciowym, aby obraz wyjœciowy niós³ ze sob¹ wiêcej informacji. Ze wzglêdu na posiadane cechy algorytymy te mo¿emy podzieliæ na pewne grupy:
\begin{itemize}
...
\end{itemize}
Z racji przyjêtej w dalszej czêœci pracy szczegó³ow skupiono sie na opisie przekszta³ceñ kontekstowych.
\section{Filtracja splotowa}

Z racji metodyki prowadzonych dzia³añ szczegó³owo opisana zostanie filtracja splotowa nale¿¹ca do 
Filtracja splotowa nale¿y do grupy przekszta³ceñ kontekstowych. Oznacza to, ¿e do wyznaczenia wartoœci pojedynczego piksela obrazu wyjœciowego potrzebne jest wykonanie obliczeñ na wielu pikselach obrazu wejœciowego. 
...
\begin{figure}[b!]
  ...
  %\label{fig:rysunek2}\source{Opracowanie na podstawie dokumentacji elektronicznej MATLAB 7.0 \cite{Matlab}}
\end{figure}
Innym przyk³adem filtru splotowego jest filtr Gabora, który zosta³ opisany w kolejnej sekcji.
\begin{figure}[h]
...
wykorzystaniem zostan¹ wyeksponowane sk³adowe widma obrazu o okreœlonej czêstotliwoœci. Jego twórc¹ jest Dennis Gabor, wêgieski laureat Nagrody Nobla w dziedzinie fizyki (1900-1979). Filtr ten wykorzystywany jest g³ównie do detekcji tekstur w obrazach, oraz przy wykrywaniu krawêdzi. W cyfrowym przetwarzaniu obrazów mo¿na go realizowaæ w dziedzinie przestrzennej poprzez dyskretn¹ dwuwymiarow¹ konwolucjê z wyliczon¹ mask¹ lub przez zwyk³e mno¿enie w dziedzinie czêstotliwoœci (wykorzystujemy tutaj twierdzenie o splocie).
